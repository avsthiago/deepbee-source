...
def cross_plataform_directory():
    global PATH_IMAGES, PATH_DETECTIONS, PATH_PREDICTIONS, PATH_OUT_IMAGE, \
           PATH_OUT_CSV 
    if '\\' in PATH_IMAGES:
        PATH_IMAGES += '\\'
        PATH_DETECTIONS += '\\'
...
def get_qtd_by_class(points, labels):
    ...
    return [*[str(sum_predictions[i]) for i, j  in enumerate(labels)],str(len(points_filtered)) ]
...
def draw_labels_bar(image, labels, colors):
    ...
    for i, cl in enumerate(zip(colors, labels)):
        color, label = cl
        cv2.putText(left_panel, " ".join([str(i+1), ".", label]), 
                    (15, 70*(i+1)), cv2.FONT_HERSHEY_DUPLEX, 1.4, color, 2)    
...
def draw_circles_labels(image, labels, points, colors=None, draw_labels=True):
    if colors is None:
        ...
        colors = [(255, 0, 0), (0,255,255), (0,0,128), (255, 0, 255),
              (0,255,0),(255, 255, 100), (0,0,255)]
...
def extract_circles(image, pts, output_size=224,
    ...
    if standardize_radius:
        ...
        ROIs = [cv2.resize(img_w_border[i[1]-i[2]:i[1]+i[2], 
                             i[0]-i[2]:i[0]+i[2]], (224,224)) for i in pts]
...
def classify_image(im_name, npy_name, labels, net, img_size, file):
    try:
        ...
        for chunk in [blob_imgs[x:x + batch_size] for x in range(0, len(blob_imgs), batch_size)]:
        ...
        csl = np.vstack([i for i in [new_class, st_use_retrain, 
                                     inside_roi, vals_predictions]]).T
        
        points_pred = np.hstack((points_pred, csl))
        
        if file is not None:
            file.write(",".join([im_name.split('/')[-1], 
                                 *get_qtd_by_class(points_pred, labels)])+'\n')
        ...
        if PurePath(im_name.replace(PATH_IMAGES, '')).parts[:-1]:
            dest_folder = os.path.join(PATH_PREDICTIONS, \
                          os.path.join(*PurePath(im_name.replace(PATH_IMAGES, '')).parts[:-1]))
        else:
        ...
        out_img_name = os.path.join(PATH_OUT_IMAGE,
                                    im_name.replace(PATH_IMAGES, ''))
...
def segmentation(img, model):
    ...
    for j, x in enumerate(preds):
        RESULT_Y[j] = np.expand_dims(cv2.resize(x,(512, 512), 
                              interpolation=cv2.INTER_LINEAR)[15:497,15:497], axis=-1)
   
    reconstructed_mask = np.squeeze(np.hstack([np.vstack(i) for i in np.split(RESULT_Y, 13)]))[169:4169, 133:6133] * 255
    #reconstructed_mask *= 255
    
    if original_shape != (4000, 6000):
        reconstructed_mask = cv2.resize(reconstructed_mask, (original_shape[1],original_shape[0]))
    
    #reconstructed_mask = ((reconstructed_mask > 70) * 255).astype(np.uint8)
    
    # remove areas internas
...
def find_circles(im_name, img, mask, cnt):
    try:
        ...
        for j in range(5,50, 5):
            points = cv2.HoughCircles(roi, cv2.HOUGH_GRADIENT, dp=2, minDist=12, param1=145, 
                                  param2=55, minRadius=j+1, maxRadius=j+5)
        ...
        points = cv2.HoughCircles(roi, cv2.HOUGH_GRADIENT, dp=3, minDist=minDist, 
                                  param1=100, param2=25, minRadius=minRadius, 
                                  maxRadius=maxRadius)
        ...
        array_name =  os.path.join(* [PATH_DETECTIONS] + \
                                   list(PurePath(im_name).parts[:-1])
                                   + np_name) 
        create_folder(array_name)
        np.save(array_name, points)
    except:
        print('Cell detection failed on image ' + \
              PurePath(im_name).parts[-1]+'\n')
...
def load_dict_model(path):
    ...
    model['model_h5'] = os.path.join(path, 
                         list(filter(lambda x: "classification" in x, files))[0])
    
    model['border'] = os.path.join(path, 
                         list(filter(lambda x: "segmentaion" in x, files))[0])
    
    model['labels'] = ['Capped', 'Eggs', 'Honey', 'Larves', 
                       'Nectar', 'Other', 'Pollen']
    return model

def get_net(model, use_gpu=True):
    return 
...
def classify_images():
    ...
    detections = [os.path.join(PATH_DETECTIONS, 
                               find_image_detections(i).replace(PATH_IMAGES, "")) 
                               for i in images]
