...
def load_data(list_files):
    if os.path.isfile(list_files[-1]):
        data_loaded = np.load(list_files[-1])
    else:
        data_loaded = np.load(list_files[-2])
...
def process_entries(img_det):
    ...
    #adjust_gamma(img, GAMMA/2 if GAMMA > 0 else 0.1)
    img = process_trackbar(img)
...
def process_trackbar(img):
    global REFRESH, IM_NAME, OLD_GAMMA, OLD_BRIGHTNESS, \
           OLD_CONTRAST, REFRESH_RECTANGLE
...
def trackbar(x):
    global REFRESH, GAMMA,  BRIGHTNESS, CONTRAST, IM_NAME, OLD_GAMMA, \
           OLD_BRIGHTNESS, OLD_CONTRAST
